...
export default function ScanView() {
    ...
    return (
      <Layout>
        <div className="flex-1 flex flex-col items-center justify-center p-4">
          ...
          <p className="font-mono text-muted-foreground mb-8 max-w-md text-center truncate px-4">{scan.url}</p>
  ...
  const legalFindings = findings.filter((f: any) => f.category === "legal");
  const privacyFindings = findings.filter((f: any) => f.category === "privacy");
  const carbonFindings = findings.filter((f: any) => f.category === "carbon");
    ...
    <Layout>
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8">
            <div>
              ...
              <h1 className="text-3xl md:text-5xl font-bold font-mono tracking-tight mb-2 truncate max-w-3xl">{scan.url}</h1>
            ...
            <div className="flex items-center gap-4 no-print">
              ...
              <Button onClick={() => window.print()} size="sm" className="font-mono">
      ...
      <div className="container mx-auto px-4 py-12 flex-1">
        <Tabs defaultValue="all" className="w-full">
          ...
          <TabsContent value="all" className="m-0"><FindingsList findings={findings} /></TabsContent>
          <TabsContent value="legal" className="m-0"><FindingsList findings={legalFindings} /></TabsContent>
          <TabsContent value="privacy" className="m-0"><FindingsList findings={privacyFindings} /></TabsContent>
          <TabsContent value="carbon" className="m-0"><FindingsList findings={carbonFindings} /></TabsContent>
        </Tabs>
      </div>
    </Layout>
  );
...
function FindingsList({ findings }: { findings: any[] }) {
  ...
  return (
    <div className="grid grid-cols-1 gap-4">
      {sorted.map((finding: any) => (
        <div key={finding.id} className="border border-border/50 bg-card p-6 relative overflow-hidden hover:border-primary/30 transition-colors">
          <div className={`absolute left-0 top-0 bottom-0 w-1 ${getSeverityColor(finding.severity)}`} />
          <div className="flex items-start justify-between gap-4 mb-4">
            <div className="flex items-center gap-3">
              <span className={`font-mono text-xs uppercase px-2 py-0.5 font-bold ${getSeverityBadge(finding.severity)}`}>{finding.severity}</span>
              <span className="font-mono text-xs text-muted-foreground uppercase tracking-widest border border-border px-2 py-0.5">{finding.category}</span>
            </div>
          </div>
